...
<br>
Coorelations between mortality rate and tissue health exists.  Linear regression is used below to plot the relationship.  I use an ANOVA test to confirm the model is statistically significant. 
<br>
...
acer.port %>% 
  group_by(geno) %>% 
  count() %>%
  ggplot(aes(x = factor(geno), y = n)) +
  geom_col()

acer.dist.table <- acer.port %>% 
  count(geno, sort = TRUE) %>%
...
plot_ly(acer.port.agg, x = ~sd, 
                    ...
                    layout(xaxis = list(title = "Standard Deviation"),
                           ...
                           title = "How Risk and Return Relate") %>%
                    hide_legend()
...
summary(acer.port.agg[,2:5])
summary(acer.boot.agg[,2:5])
