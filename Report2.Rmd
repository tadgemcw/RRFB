...
acer_comp1 %>%
  plot_ly(x = ~factor(geno), y = ~round(mortality_rate,2)) %>%
  add_markers(marker = list(color = toRGB("gray15"), size = 5)) %>%
  add_bars(data = acer_comp2, color = ~factor(geno), frame = ~duration) %>%
  layout(xaxis = list(title = "Acer Genotype"),
         yaxis = list(title = "Mortality Rate"),
         title = "<br>What is the change in mortality rate over time?") %>%
  hide_legend()
...
acer_all %>%
  ...
  ggplot(aes(x = factor(geno), y = mortality_rate)) +
    ...
    scale_color_discrete(labels = c("3-Month", "6-Month")) +
    labs(x = "Acer Genotype",
         y = "Mortality Rate",
         color = "Survey",
         title = "What is the change in mortality rate over time?",
        caption = "Data from Punt Vierkant, Yellowman A, and Yellowman B")
```
```{r acer comparison 3- to 6-month}

acer_all %>%
  tissue_health() %>%
  plot_ly(x=~factor(geno), y=~weighted_health, color = ~site)
