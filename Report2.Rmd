...
The plots below highlight differences on the genomic level.  Data was utilized from each site.  When multiple surveys were provided for a site, the latest (most recent) data was used.  Specifically, the three-months surveys for Punt Vierkant, Yellowman A, and Yellowman B were omited in favor of using the six-month survey data.  The result is a snap shot of the most current findings.
...
acer_comp2 %>%
  plot_ly(x = ~factor(geno), 
          y = ~mortality_rate, 
          hoverinfo = "text", 
          text = ~paste(round(mortality_rate,2), "%")) %>%
  ...
  layout(xaxis = list(title = "Acer Genotype"),
         ...
         title = "<br>How does mortality rate change over 3 months?") %>%
...
plot <- acer_all %>%
  tissue_health() %>%
  ggplot(aes(x = factor(geno), y = weighted_health, color = site)) + 
    geom_point()
