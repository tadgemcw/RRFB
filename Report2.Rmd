...
The table below lists the summary statistics for each Acer site.
...
acer_comp2 %>%
  plot_ly(x = ~factor(geno), 
          y = ~mortality_rate, 
          hoverinfo = "text", 
          text = ~paste0("Mortality Rate: ", round(mortality_rate,2), "%")) %>%
...
acer.port %>% 
  ...
  plot_ly(x = ~factor(geno), y = ~n, color = ~factor(geno), hoverinfo = "text",
          text = ~paste0("Observations: ", n, "<br>",
...
The Apal data consists of survey from four sites: Carl's Hill, Knife I (A-C), Oil Slick, and Southbay II.  
...
The table below lists the summary statistics for each Apal site.
...
apal_comparison_sites <- c("Carl's Hill", "Knife B", "Knife C")

apal_comp1 <- apal_all %>%
  filter(site %in% apal_comparison_sites) %>% 
  tissue_health() %>%
  group_by(species, duration, geno) %>%
  summarize(num_outplanted = sum(n_outplanted),
            num_survived = sum(n_present, na.rm = TRUE) - sum(health_0, na.rm=TRUE),
