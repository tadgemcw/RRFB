...
acer_all <- acer_all %>%
  tissue_health() %>%
  mutate(mortality_rate = (1-(n_present - health_0) / n_outplanted) * 100)
acer_all$mortality_rate <- replace_na(acer_all$mortality_rate, 0)

plot_ly(acer_all, x = ~mortality_rate) %>%
  add_histogram()

str(acer_all$mortality_rate)
