...
Today, coral reef restoration is largely under valued and under funded by much of the world.  Yet our ability to propagate and grow new coral reefs is a vital piece in the fight to manage our changing climate.  From this imbalance, the realities and constraints of operational resources cannot be ignored.  Outplanting new reefs must be done with maximum efficiency.  
The feature of this report attempts to utilize financial portfolio analysis to construct a genomic portfolio from which organizations may optimize their resources and outplant new reefs with increased efficiency.  The models developed for two coral species show that improvements to overall reef survival rates are possible when the proportions of genotypes are weighted to achieve the desired portfolio goals.  More analysis is needed support the long term success and tuning of such a portfolio; however, the models suggest that immediate gains can be made.

## Methodology

The basic analysis is intended to adhere to the guidance proposed by the National Oceanic & Atmospheric Association (NOAA) in its September 2020 technical memorandum *Coral Reef Restoration Monitoring Guide: Methods to Evaluate Restoration Success From Local Ecosystem Scales.* All data was provided by RRB.  The ecological footprint data was collected from photomosaics and traditional underwater surveying methods.  The site-specific data was provided from detailed underwater surveys of the outplanted reefs  
...
All calculations and visualizations are performed using R.  R packages for this project include *knitr*, *readxl*, *tidyverse*, *plotly*, *flextable*, and *PortfolioAnalytics*.  The report is formatted in Markdown, and all versions have been pushed to GitHub.
...
The following visuals highlight the changes in ecological footprints and total coral coverage from year to year.  Data from RRB's long-term surveys was used for this section.  As new outplanting sites are created each year, not all sites have been equally surveyed.  
...
In the visual below, I have plotted the change in Coral Cover as a percentage by site.  As previously noted, this calculation is the total area of coral cover, divided by the size of the eco-footprint.  It represents the density of coral coverage for a given site.
...
The plots below highlight differences on the genomic level.  Data is utilized from each site.  When multiple surveys were provided for a site, the latest (most recent) data was used to avoid duplication.  Specifically, the three-month surveys for Punt Vierkant, Yellowman A, and Yellowman B were omited in favor of using the six-month survey data.  The result is a snap shot of current conditions.
...
Separating the mortality rate findings by site reveals which sites have performed better than others.  That said, it is important to note that not all sites contain all genos.  Hence, site statistics may be unfairly skewed by a greater number of poorly performing genos as the they cannot be held equal.
<br>
...
The average tissue health for each geno is generally in line with the mortality rates.  Genos #9, #14, and #22 have not performed as well as others in either measure.  It is worth mentioning that genos #3 and #6 (each with higher mortality rates) have good average tissue health if a few outliers are omitted.
...
The goals of Reef Renewal Bonaire are to successfully outplant as many corals as possible but also to ensure new reefs are genomicaly diverse.  Thus, the driving question is how to achieve both goals with maximum efficiency of time and resources.  Though one strategy would be to outplant an equal number of corals from each genotype, the data shows that not all genotypes are equally resilient.  Because of this, inefficiencies are introduced.  Therefore, I propose building a weighted portfolio of genotypes in the same manor as one might build a financial portfolio of investments.

In typical portfolio analysis, the primary factors of consideration are the return on investment for each account and the risk (typically, the standard deviation of average returns).  From these factors, an optimal basket of accounts is selected to achieve the desired portfolio goals.  In a portfolio of genotypes, I propose using the inverse mortality rate (the survival rate) as the return on investment.  For example, geno #1 has an average mortality rate of 3.11%; therefore, a 96.89% survival rate is the return on investment.  Similarly, geno #3 has a 21.54% average mortality rate and so there is a 78.46% average return.  The standard deviation of the average returns is then calculated for each genomic group as a measure of risk.

Below, I have plotted the distribution of observations to visualize the raw sample size of each genotype.  
...
Having a sufficiently large sample size to calculate accurate returns and standard deviations is important.  Because the sample size of some genotypes is limited, I use bootstrapping to artificially expand the sample size.  Each entire genotype group is randomly sampled 1000 times with replacement.  The result is a larger sample of the original data.  Through the Central Limit Theorem, the bootstrapped data approaches a normal distribution.  Thus, the distribution of returns are closer to the sample means and the risk factors (standard deviations) are more closely representative of the population.
...
I use the Portfolio Analytics' *optimize.portfolio* function on the bootstrapped data set to calculate the optimal weights of each genotype in the portfolio.  Portfolio specifications are added to ensure that 1)the portfolio is fully invested and long-only, whereby the sum of weights total to 1.0 (or 100%), 2)returns are defined as the mean survival rate and risk is defined as the standard deviation from the means, and 3)each genotype makes up a minimum of 1% of the total portfolio and a maximum of 25%.

The outputs below identify the optimal weights for each genotype as well as the anticipated portfolio return and risk.  Additionally, the graph plots the optimal portfolio against all other iterations of the model.  
...
The model predictions are improvements on both survival rate (mean return) and risk (standard deviation of average returns).  A comparison between the existing values and the optimized portfolio is below.  The optimized portfolio model suggests that improvements to both survival rate and risk can be made by following the optimal portfolio weights for future outplantings.  
...
The Apal coral speices, commonly known as Elkhorn coral, has been outplanted at the following sites: Carl's Hill, Knife (subsites A-C), Oil Slick Leap, and South Bay II.

The Carl's Hill, Knife B, and Knife C surveys were taken three- and six-months after outplanting.  Data was provided from one-, five-, and six-month surveys for Knife A.  Data from Oil Slick Leap was collected one-, seven-, and 13-months after outplanting, and the Southbay II data was collected at one half-month and 18 months from outplanting.
...
Genos #16, #17, and #22 have the highest mortality rates.  Unlike the Acer date, these genos do not clearly correspond with the average tissue health statistics. Conversely, genos #9-10, and #13 have performed better than the rest.  
...
As with the Acer data, I filter the Apal data to sites with both three- and six-month surveys (Carl's Hill, Knife B and Knife C) and plotted the change in mortality rate over time.  

It should be noted that instances in which the mortality decreased also correspond with a decrease in sample size.  For example, the three-month surveys of geno #23 show a total of 20 outplanted observations; however the six-month surveys show a total of only 10 outplanted observations.  It appears that not all clusters were re-surveyed.  Therefore, the comparison over time is not a perfect comparison of the same samples.
...
Separating the mortality rate findings by site reveals which site have performed better than others.  Genos #6-11 have more stable mortality rates across sites than, say, genos #2, 19, and 22.
<br>
*Note: clicking on the legend names allows you to filter the chart.*
...
The average tissue health for the Apal species is slightly higher than the Acer species.  Genos #2, #8, and #14 have not performed as well as others; however, the remaining genos are more tightly grouped, excluding outliers.
...
The process for modeling the Apal portfolio is the same as for the Acer data.  The raw data is reviewed, bootstrapped, and modeled with the same portfolio weight requirements.
...
Again, via the Central Limit Theorem, the bootstrapped data approaches a normal distribution.  Thus, the distribution of returns closely resemble the sample means and the risk factors (standard deviations) used for portfolio analysis are more closely representative of the population.
...
The same portfolio constraints and objectives are used for the Apal data.  The portfolio is constructed with a full long-only requirement; weights are set to a minimum of 1% and a maximum of 25%.
...
The result of the optimized portfolio suggests that improvements to survival rate and risk can be made to future outplantings.  
...
Our ability to propagate and grow new coral reef systems is a vital piece in the fight to manage our changing climate and slow the ill-effects of pollution.  Yet at this time, coral reef restoration is largely under valued and under funded by much of the world.  While maintaining genomic diversity is, and should be, an important aspect of fostering healthy reefs, the realities and constraints of resources cannot be ignored.  Outplanting new reefs must be done with maximum efficiency.  
The genomic portfolio I propose is one way in which organizations may optimize their resources to build reefs with a new level of efficiency.  The models above show that improvements to overall reef survival rates are possible with decreased risk.  Additionally, the models do not consider growth rates and natural reproduction cycles of each species.  This information could potentially yield another dimension to the model by which a long term diversity strategy could be developed.  While more research is clearly needed for data collection and model tuning, a 1% improvement today has the potential to grow exponentially.  As in finance, coral restoration has the ability to grow from its own gains.  
