...
```{r attempting heat map of mortality and tissue health}
...
acer_latest %>%
  ...
  summarize(num_outplanted = sum(n_outplanted),
            ...
            avg_health = mean(weighted_health)) %>%
  arrange(mortality_rate) %>%
  ggplot(aes(x = mortality_rate, y = avg_health)) +
    ...
    labs(x = "Mortality Rate",
         ...
         color = "Site",
         shape = "Site",
         title = "Correlation Between Tissue Health & Mortality Rate")
...
```{r acer comparison 3- to 6-month}
