...
## Summary
The goal of this project is to assist [Reef Renewal Bonaire](https://www.reefrenewalbonaire.org) (RRB) in developing a successful, data driven program to efficiently introduce new, "reef-ready" corals into Bonaire's reef systems.  By analyzing survey data from multiple coral reef outplanting locations, I attempt to provide insights into the viability of two coral species, their genotypes, and the overall changes to the ecological footprints of the restored reef locations.  Special interest was given to the performance of coral growth on the geno-level.  The successful propagation and growth of multiple genotypes is central to the diversity of the species.  Additionally, if certain genotypes prove to be more resistant to temperature fluctuation than others, they may become vitual to the long-term success of the species as we face the realities of climate change.  Through mortality and average tissue health calculations, I was able to identify which genotypes are performing better than others and begin to track the changes in tissue health over time.  While more analysis is needed, I hope this project will lead to the development of future models from which RRB and others can further their work.
...
The summary statistics below were generated from a survey of coral health between April 11th, 2021 to May 4th, 2021, approximately three months after the initial outplanting of corals at Carl's Hill.  Note that Geno #7 appears to have a negative mortality rate.  This statistic was caused because more observations were recorded during the three-month survey than were originally outplanted.  It is unknown whether this was caused by a division of corals or a survey error.
...
```{r, carls hill 3 month weighted coral health by geno, message=FALSE}
carl_health <- carl3 %>%
...
```{r carls hill 3 month mortality}
ggplot(carl3_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Carl's Hill 3-Month Mortality Rate") + 
...
```{r, carls hill boxplot of tissue health by geno}
...
The following data was taken three months later in a survey conducted on July 4th, 2022.   When compared with the three-month survey, some genos have performed better than others.  Genos #14-15 continue to maintain high mortality rates.  Genos #6-7, #9 and #22-23 improved in average tissue health, while the remaining genos declined in health.  


```{r carls hill 6 month weighted health, message=FALSE}
carl6_health <- carl6 %>%
  select(species, geno, n_outplanted, n_present, health_0, health_q1, health_q2, health_q3, health_q4, health_100) %>%
  rowwise() %>%
  mutate(Health0 = sum(health_0, na.rm = TRUE),
...
```{r carls hill 6 month mortality}
ggplot(carl6_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Carl's Hill 6-Month Mortality Rate")+ 
  theme(legend.position = "none")
...
```{r carls hill 6 month boxplot of tissue health}
...
```{r carls hill comparing surveys}
...
```{r, yellowman3A calculating A3 weighted coral health by geno, message=FALSE}
...
```{r Yellowman A 3 Month mortality}

ggplot(yellowmanA3_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Yellowman A 3-Month Mortality Rate")+ 
  theme(legend.position = "none")
...
```{r, yellowmanA3 boxplot of tissue health by geno}
...
After six months, mortality rates are still quite low.  The exception is Geno #22, which continues to decline.


```{r yellowmanA6 calculating 6A weighted coral health by geno, message=FALSE}
...
```{r Yellowman A 6 month mortality}
ggplot(yellowmanA6_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Yellowman A 6-Month Mortality Rate")+ 
  theme(legend.position = "none")
```
...
```{r yellowman6A boxplot of tissue health by geno}
...
```{r yellowman A comparing surveys}
...
At Yellowman B, RRB outplanted different genotypes from those at Yellowman A.  Only geno #1 is shared between Yellowman A and Yellowman B.


```{r yellowmanB3 calculating B3 weighted coral health by geno, message=FALSE}
...
```{r Yellowman B 3 month mortality}
ggplot(yellowmanB3_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Yellowman B 3-Month Mortality Rate")+ 
  theme(legend.position = "none")
```
...
```{r yellowmanB3 boxplot of tissue health by geno}
...
The six-month survey of Yellowman B shows quite a low mortality rate, though Geno #9 is increasing.


```{r yellowmanB6 calculating B6 weighted coral health by geno, message=FALSE}
...
```{r Yellowman B 6 month mortality}
ggplot(yellowmanB6_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Yellowman B 6-Month Mortality Rate")+ 
  theme(legend.position = "none")
```
...
```{r yellowmanB6 boxplot of tissue health by geno}
...
```{r yellowman B comparing surveys}
...
```{r, yellowman C 3-Month weighted coral health by geno, message=FALSE}
...
```{r Yellowman C 3 Month Mortality}
ggplot(yellowmanC3_agg, aes(x=factor(geno), y=mortality_rate, fill=factor(geno))) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Genotype",
       title="Yellowman C 3-Month Mortality Rate")+ 
  theme(legend.position = "none")
```
...
```{r yellowmanC3 boxplot of tissue health by geno}
...
yellowmanA3C3_agg <- yellowmanA3C3_health %>% 
  filter(geno %in% yellowmanA3_health$geno & geno %in% yellowmanC3_health$geno) %>%
  group_by(species, geno, location) %>%
    summarize(num_outplanted = sum(n_outplanted),
              num_survived = sum(n_present, na.rm = TRUE) - sum(health_0, na.rm=TRUE),
              mortality_rate = 100 - sum(num_survived / num_outplanted)*100,
              avg_health = mean(weighted_health))

ggplot(yellowmanA3C3_agg, aes(x=factor(geno), y=mortality_rate, fill=location)) +
  geom_col() +
  scale_y_continuous(limits = c(-10,80), labels=scales::percent_format(scale=1)) +
  scale_fill_discrete(labels=c('Yellowman A', 'Yellowman C')) +
  labs(x="Genotype", 
       y="Mortality Rate",
       fill="Location",
       title="Yellowman B 6-Month Mortality Rate")
...
```{r Comparing YellowmanA 3 with Yellowman C 3}
...
```{r, carl 3 bottom stressor proportion}
c3bottom_perc <- carl3 %>%
...
```{r carl 6 bottom stressor proportion}
c6bottom_perc <- carl6 %>%
  summarize(N_Outplanted = n(),
            Total_Stressors = sum(n_stressors, na.rm = TRUE),
            Bottom_Contact = sum(bottom_contact, na.rm = TRUE),
...
```{r opportunities yellowmanB3 bottom contact}
...
```{r opportunities yellowmanB6 bottom contact}
...
```{r pportunities yellowmanC3 bottom contact}
