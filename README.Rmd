...
**Comparing Average Tissue Health Between Acer Sites After 3-Month Surveys**
...
ggplot(yellowmanA3C3_health, aes(x = factor(geno), y = weighted_health, color = location)) + 
  geom_point() +
...
```{r, eco growth by metric, echo=FALSE}
...
yb3bottom_perc <- yellowmanB3 %>%
  summarize(N_Outplanted = n(),
            ...
            Percent_of_Total = (sum(yellowmanB3$bottom_contact, na.rm = TRUE) / 
                sum(yellowmanB3$n_stressors, na.rm = TRUE) * 100))
